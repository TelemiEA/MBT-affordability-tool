-- MBT Affordability Tool - Supabase Database Schema
...
CREATE TABLE automation_runs (
    ...
    run_type TEXT NOT NULL, -- 'normal', 'credit', 'full'
...
CREATE TABLE scenario_results (
    ...
    -- Foreign key to automation_runs
...
-- Function: Calculate average Gen H rank across all runs
CREATE OR REPLACE FUNCTION calculate_average_gen_h_rank()
RETURNS DECIMAL(5,2)
LANGUAGE sql
AS $$
    SELECT COALESCE(AVG(average_gen_h_rank), 0)
    FROM automation_runs
    WHERE average_gen_h_rank > 0;
$$;

-- Function: Get best performing scenario (lowest average rank across all runs)
...
AS $$
    SELECT 
        ar.run_timestamp,
        ar.run_type,
        COALESCE(AVG(sr.gen_h_vs_average_gap), 0) as average_gap
...
AS $$
    SELECT 
        scenario_id,
        ROUND(AVG(gen_h_rank), 2) as avg_rank,
...
-- CREATE POLICY "Enable read access for all users" ON automation_runs FOR SELECT USING (true);
-- CREATE POLICY "Enable insert access for all users" ON automation_runs FOR INSERT WITH CHECK (true);
-- CREATE POLICY "Enable read access for all users" ON scenario_results FOR SELECT USING (true);
-- CREATE POLICY "Enable insert access for all users" ON scenario_results FOR INSERT WITH CHECK (true);
